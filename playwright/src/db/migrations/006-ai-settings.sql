...
INSERT INTO ai_tools (id, name, description, category, enabled, parameters) VALUES
  ...
  ('get_qc_checklists', 'Get QC Checklists', 'Discovers and returns all QC checklist files. Each checklist defines formal test steps and expected results for a specific interaction type (e.g., Extended Text, Inline Choice, Matching). Use this tool when generating QC checklist tests — it returns the full content of every available checklist so you can select the right one.', 'data', true, '{}'),
  ('get_test_template', 'Get Test Template', 'Returns a pre-filled test script skeleton for a given test type. The template is auto-configured with the correct helpers, imports, item counts, and platform-specific patterns based on the current test context (assessment, environment). Use this as the FIRST step when generating any test — it eliminates the need to call list_helpers, read_file, or get_items. Types: baseline, ai_content, ai_visual, qc_checklist, functional, visual_comparison.', 'editing', true, '{"required": ["type"]}')
